...
if __name__ == '__main__':
	...
	session = cc.database.create_session('/home/156/wf4500/databases/access/panan0025_rerun.db')
	...
	temp_xflux = cc.querying.getvar(exp,'T_adx',session,ncfile='%month_rho2.nc',start_time=start_time,end_time=end_time).sel(time=start_time)
	temp_yflux = cc.querying.getvar(exp,'T_ady',session,ncfile='%month_rho2.nc',start_time=start_time,end_time=end_time).sel(time=start_time)
	...
	#Converting speeds into transports volcello
	vhrho_nt = cc.querying.getvar(exp,'vmo',session,ncfile='%month_rho2.nc',start_time=start_time,end_time=end_time).sel(time=slice(start_time,end_time))
	uhrho_et = cc.querying.getvar(exp,'umo',session,ncfile='%month_rho2.nc',start_time=start_time,end_time=end_time).sel(time=slice(start_time,end_time))
	...
	vmo = cc.querying.getvar(exp,'vmo',session,ncfile='%month_rho2.nc',frequency='1 monthly',start_time=start_time,end_time=end_time).sel(time=slice(start_time,end_time)).isel(time=0)
	umo = cc.querying.getvar(exp,'umo',session,ncfile='%month_rho2.nc',frequency='1 monthly',start_time=start_time,end_time=end_time).sel(time=slice(start_time,end_time)).isel(time=0)
	...
	temp_xflux=temp_xflux.isel(xq=slice(1,xmax)).isel(yh=slice(0,ymax)).mean('time')
	
	temp_yflux=temp_yflux.isel(yq=slice(1,ymax+1)).mean('time')
