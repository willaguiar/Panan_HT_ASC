...
if __name__ == '__main__':
	...
	Salt_trans_zonal=Salt_trans_zonal.isel(xq=slice(1,xmax))
	Salt_trans_meridional=Salt_trans_meridional.isel(yq=slice(1,ymax))
	...
	Salt_trans_zonal = Salt_trans_zonal.compute()
	Salt_trans_meridional = Salt_trans_meridional.compute()
	print('Lets do another checking')
	print('Salt_trans_zonal = ')
	print(Salt_trans_zonal)
	print('Salt_trans_meridional = ')
	print(Salt_trans_meridional)
	...
	print('Most of the imports and calculations are done. Now we need to select the data on the contour')
	...
	Salt_trans_meridional_mask_month=Salt_trans_meridional
	Salt_trans_meridional_mask_month['yq'] = np.array(mask_y_transport.y_ocean)
	Salt_trans_meridional_mask_month['xh'] = np.array(mask_y_transport.x_ocean)
	Salt_trans_meridional_mask_month = Salt_trans_meridional_mask_month.rename(yq='y_ocean',xh='x_ocean')*mask_y_transport
	
	Salt_trans_zonal_mask_month=Salt_trans_zonal
	Salt_trans_zonal_mask_month['yh'] = np.array(mask_y_transport.y_ocean)
	Salt_trans_zonal_mask_month['xq'] = np.array(mask_y_transport.x_ocean)
	Salt_trans_zonal_mask_month = Salt_trans_zonal_mask_month.rename(yh='y_ocean',xq='x_ocean')*mask_y_transport
	
	
	Salt_trans_meridional_mask_month['y_ocean']=np.array(mask_y_transport_numbered.y_ocean)
	Salt_trans_meridional_mask_month['x_ocean']=np.array(mask_y_transport_numbered.x_ocean)
	
	Salt_trans_zonal_mask_month['y_ocean']=np.array(mask_x_transport_numbered.y_ocean)
	Salt_trans_zonal_mask_month['x_ocean']=np.array(mask_x_transport_numbered.x_ocean)
	...
	Salt_trans_meridional['yq']=np.array(mask_y_transport_numbered.y_ocean)
	Salt_trans_meridional['xh']=np.array(mask_y_transport_numbered.x_ocean)
	
	Salt_trans_zonal['yh']=np.array(mask_x_transport_numbered.y_ocean)
	Salt_trans_zonal['xq']=np.array(mask_x_transport_numbered.x_ocean)
	...
	# stack transports into 1d and drop any points not on contour:
	x_along_1d = Salt_trans_zonal.drop('xh').rename(yh='y_ocean',xq='x_ocean').stack(contour_index = ['y_ocean', 'x_ocean'])
	x_along_1d = x_along_1d.where(mask_x_numbered_1d>0, drop = True)
	y_along_1d = Salt_trans_meridional.drop('yh').rename(yq='y_ocean',xh='x_ocean').stack(contour_index = ['y_ocean', 'x_ocean'])
	y_along_1d = y_along_1d.where(mask_y_numbered_1d>0, drop = True)
	
	# combine all points on contour:
	salt_transp_across_contour = xr.concat((x_along_1d, y_along_1d), dim = 'contour_index')
	salt_transp_across_contour = salt_transp_across_contour.sortby(contour_ordering)
	salt_transp_across_contour.coords['contour_index'] = contour_index_array
	salt_transp_across_contour = (salt_transp_across_contour*transp_dir).load()
	...
	cross_slope_salt_trans = np.zeros([ndays_month,51,n_bins])
	for lon_bin in range(n_bins):
		salt_trans_this_bin0 = salt_transp_across_contour[:,:,bin_edge_indices_west[lon_bin]:bin_edge_indices_east[lon_bin]]
		cross_slope_salt_trans[:,:,lon_bin] = np.sum(salt_trans_this_bin0,axis=2)
	...
	print('Calculating Zonal salt convergence')
	...
	zonal_salt_trans_west = np.zeros([ndays_month,51,n_bins])
	for lon_bin in range(n_bins):
		# west limit:
		...
		if on_x_grid == True:
			zonal_salt_trans_west[:,:,lon_bin] = np.sum(Salt_trans_zonal[:,:,:jj,ii],axis=2)
		#in this case we want transport half a grid point to the west:
			...
			if ii==0:
				zonal_salt_trans_west[:,:,lon_bin] = np.sum(Salt_trans_zonal[:,:,:jj+1,-1],axis=2)
			else:
				zonal_salt_trans_west[:,:,lon_bin] = np.sum(Salt_trans_zonal[:,:,:jj+1,ii-1],axis=2)
	
	zonal_salt_trans_east = np.zeros([ndays_month,51,n_bins])
	for lon_bin in range(n_bins):
		# east limit:
		# reset these to False:
		on_x_grid = False
		...
		if on_x_grid == True:
			zonal_salt_trans_east[:,:,lon_bin] = np.sum(Salt_trans_zonal[:,:,:jj,ii],axis=2)
			# in this case we want transport half a grid point to the west:
		elif on_y_grid == True:
			# wrap around to east side of grid:
			if ii==0:
				zonal_salt_trans_east[:,:,lon_bin] = np.sum(Salt_trans_zonal[:,:,:jj+1,-1],axis=2)
			else:
				zonal_salt_trans_east[:,:,lon_bin] = np.sum(Salt_trans_zonal[:,:,:jj+1,ii-1],axis=2)
	
	zonal_convergence = zonal_salt_trans_east - zonal_salt_trans_west
	
	
		
	
	# PREVIOUS SAVEDIR TO LOCATE THE FILES save_dir  = '/g/data/x77/wf4500/ASC_project/ASC_speed/monthly/OM2_RYF/'
	save_dir  = '/home/156/wf4500/v45_wf4500/Project_panan/GH/Panan_HT_ASC/Processed_data/panan005/CSsaltT_daily_z/Ant_cross_slope_salt_terms_offline_'+str(isobath_depth)+'m_'+ start_time+'.nc'    
	
	
	data_ZC=xr.DataArray((zonal_convergence),name="zonal_convergence",dims=["time","zl","lon_bin_midpoints"])
	data_CSSalT=xr.DataArray((cross_slope_salt_trans),name="binned_cross_slope_salt_trans",dims=["time","zl","lon_bin_midpoints"])
	data_SalT=salt_transp_across_contour
	data_SalT.name='unbinned_salt_transp_across_contour'
	...
	data_to_saveuv = xr.merge([data_ZC,data_CSSalT,data_SalT,data_lat,data_lon,data_latbin])
	data_to_saveuv['contour_index']=salt_transp_across_contour.contour_index
	data_to_saveuv['zl']=np.array(salt_transp_across_contour.zl)
	data_to_saveuv['lon_bin_midpoints']=np.array(lon_bin_midpoints)
	data_to_saveuv['time']=salt_transp_across_contour.time
