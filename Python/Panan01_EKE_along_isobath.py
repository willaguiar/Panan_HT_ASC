...
if __name__ == '__main__':
	...
	#importing volume
	vol_centre = cc.querying.getvar(exp,'volcello',session,ncfile='%month_rho2.nc',start_time=start_time,end_time=end_time,chunks={}).sel(time=slice(start_time,end_time)).sel(yh=lat_range).sel(time=slice(start_time0,end_time0))
	...
	U_qmean=xr.open_dataset('/g/data/ik11/users/wf4500/Project_panan/GH/Panan_HT_ASC/Processed_data/panan01_rerun/EKE_along_contour_rho/U_mean_1991_2000.nc').uo
	V_qmean=xr.open_dataset('/g/data/ik11/users/wf4500/Project_panan/GH/Panan_HT_ASC/Processed_data/panan01_rerun/EKE_along_contour_rho/V_mean_1991_2000.nc').vo
	...
	#first dvdx
	dvdx =  V_yq.where( V_yq > -9e9999).diff('yq').drop_vars('yh').rename({'yq':'yh'}); 
	dvdx['yh'] = U_xq.yh;
	dvdx = (dvdx/dx_xhyh).compute()
	
	
	#now dudy
	dudy = U_xq.where( U_xq > -9e9999).diff('xq').drop_vars('xh').rename({'xq':'xh'})
	dudy = dudy.where( dudy > -9e9999)
	dudy['xh'] = V_yq.xh
