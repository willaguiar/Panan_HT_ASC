...
if __name__ == '__main__':
	...
	session = cc.database.create_session('/home/156/wf4500/databases/access/panan0025_final_ol01.db')
	#session = cc.database.create_session('/home/156/wf4500/databases/access/panan01_rerun_withCSHT.db')
	...
	if isobath_depth==1000:
		outfile = '/g/data/ik11/users/wf4500/Project_panan/GH/Panan_HT_ASC/contours/Antarctic_slope_contour_Panan0025_'+str(isobath_depth)+'m.npz'
	else:
		outfile = '/g/data/ik11/users/wf4500/Project_panan/GH/Panan_HT_ASC/contours/Antarctic_slope_contour_Panan0025_Smoothedtopography_'+str(isobath_depth)+'m.npz'
	...
	temp_xflux = cc.querying.getvar(exp,'T_adx',session,ncfile='%rho%',frequency='1 monthly',start_time=start_time,end_time=end_time).sel(time=start_time)
	temp_yflux = cc.querying.getvar(exp,'T_ady',session,ncfile='%rho%',frequency='1 monthly',start_time=start_time,end_time=end_time).sel(time=start_time)
	...
	vhrho_nt = cc.querying.getvar(exp,'vmo',session,ncfile='%rho%',frequency='1 monthly',start_time=start_time,end_time=end_time).sel(time=slice(start_time,end_time))
	uhrho_et = cc.querying.getvar(exp,'umo',session,ncfile='%rho%',frequency='1 monthly',start_time=start_time,end_time=end_time).sel(time=slice(start_time,end_time))
	
	vhrho_nt = vhrho_nt.sel(time=slice(start_time0,end_time0))
	...
	vmo = cc.querying.getvar(exp,'vmo',session,ncfile='%rho%',frequency='1 monthly',start_time=start_time,end_time=end_time).sel(time=slice(start_time,end_time)).isel(time=0)
	umo = cc.querying.getvar(exp,'umo',session,ncfile='%rho%',frequency='1 monthly',start_time=start_time,end_time=end_time).sel(time=slice(start_time,end_time)).isel(time=0)
	...
	freezing_point_heat_trans_zonal = (cp*umo*temp_freezing)
	freezing_point_heat_trans_meridional = (cp*vmo*temp_freezing)
	xmax=int(np.size(temp_xflux.xq))
	if exp == 'panant-0025-zstar-ACCESSyr2':
		ymax=int(np.size(temp_xflux.yh))
	else:
		ymax=int(np.size(temp_xflux.yh))
	...
	temp_yflux=temp_yflux.isel(yq=slice(1,ymax+1))
	...
	# compare both ways:
	temp_yflux_month = (temp_yflux - freezing_point_heat_trans_meridional).isel(time=0)
	temp_xflux_month = (temp_xflux - freezing_point_heat_trans_zonal).isel(time=0)
	
	
	
	#
	print('number of days in the current month = ' +str(ndays_month))
	print('freezing_point_heat_trans_meridional shape = ')
	print(freezing_point_heat_trans_meridional.shape)
	print('temp_yflux shape = ')
	print(temp_yflux.shape)
	
	print('temp_yflux_month shape = ')
	print(temp_yflux_month.shape)
	print('temp_xflux_month shape = ')
	print(temp_xflux_month.shape)
	...
	print(heat_transp_across_contour.shape)
	heat_transp_across_contour = (heat_transp_across_contour*transp_dir).transpose("rho2_l","contour_index").load() #transposed as a way to fix 
	#heat_transp_across_contour = (heat_transp_across_contour*transp_dir).transpose("rho2_l","contour_index","time").drop('time').load() #transposed as a way to fix some errors I had before
	#heat_transp_across_contour= heat_transp_across_contour[::,0] #fxing dimensions again
	...
	save_dir  = '/g/data/ik11/users/wf4500/Project_panan/GH/Panan_HT_ASC/Processed_data/panan0025_rerun/CSHT_month_rhoonline/Ant_cross_slope_heat_terms_offline_'+str(isobath_depth)+'m_'+ start_time+'.nc'    
